...
loadFile("/users/rrachapa/TextAnalytics-kNN-Boosting/DataSet/AmazonDataSet/amazon_baby_train.csv")
...
loadFile("/users/rrachapa/TextAnalytics-kNN-Boosting/DataSet/AmazonDataSet/amazon_baby_test.csv")
...
knc_map.fit(trainSparseMatrix, targetRating)

## Predict the test data based on the Neural Network built.
print "Predicting..."
s=f=0
print "------------------------------------------------------------\n\n"
#predictedRating = list(knc_map.predict(testDataFeatures))
#for i in range(len(predictedRating)):
#	if predictedRating[i] == testRating[i]:
#		s+=1
#	else :
#		f+=1
print "Prediction rate is "+str(knc_map.score(testDataFeatures,testRating)*100)
